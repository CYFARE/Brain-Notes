...
```bash
ulimit -n unlimited -u unlimited -s unlimited 2>/dev/null; ulimit -n 1048576 -u 1048576 -s unlimited && sudo cpupower frequency-set -g performance && echo performance | sudo tee /sys/devices/system/cpu/cpu*/cpufreq/scaling_governor && sudo sysctl -w vm.swappiness=10 vm.dirty_ratio=20 vm.dirty_background_ratio=10 kernel.sched_autogroup_enabled=0 && sudo systemctl stop packagekit ananicy-cpp 2>/dev/null; sudo cpupower idle-set -D 0
```
